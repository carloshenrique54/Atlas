...
function Home() {
  ...
  const [detalhesContato, setDetalhesContato] = useState("");
  const [categoriaContato, setCategoriaContato] = useState("")
  const [assuntoContato, setAssuntoContato] = useState("")

  const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

  const [mensagemErroContato, setMensagemErroContato] = useState(false)
  const [mensagemContato, setMensagemContato] = useState(false)
  

  async function EnviarMensagem(e){
    e.preventDefault()
    if (!nomeContato){setMensagemErroContato("Insira seu nome");return}
    if (!sobreNomeContato){setMensagemErroContato("Insira seu sobrenome");return}
    if (!emailContato){setMensagemErroContato("Insira seu email");return}
    if (!detalhesContato){setMensagemErroContato("Insira sua mensagem");return}
    if (!categoriaContato){setMensagemErroContato("selecione uma categoria");return}
    if (!assuntoContato){setMensagemErroContato("Insira seu assunto");return}

    const {data: erroContato} = await supabase
      .from("mensagens_contato")
      .insert([{nome: nomeContato, sobrenome: sobreNomeContato, email: emailContato, categoria: categoriaContato, assunto: assuntoContato, mensagem: detalhesContato, status: "novo"}])
      if (erroContato){
        alert("Erro: ", erroContato)
        return
      }

      setMensagemContato("Mensagem enviada com sucesso! Cheque sua caixa de e-mail");
      await delay(5000)
      setMensagemContato(false);
      return
  }
          ...
          <div className="contact-layout">
            ...
            <div className="contact-form-col">
              <div className="contact-form-card reveal">
                ...
                <div className={!mensagemErroContato ? "alertContato" : "alertContato ativo"}>
                  <p>{mensagemErroContato}</p>
                </div>
                <div className={!mensagemContato ? "corretoContato" : "corretoContato ativo"}>
                  <p>{mensagemContato}</p>
                </div>
                <form onSubmit={EnviarMensagem}>
                  <div className="form-row">
                    <div className="form-group">
                      <label>Nome</label>
                      <input value={nomeContato} onChange={e => setNomeContato(e.target.value)} type="text" placeholder="Seu nome" />
                    </div>
                    <div className="form-group">
                      <label>Sobrenome</label>
                      <input value={sobreNomeContato} onChange={e => setSobreNomeContato(e.target.value)} type="text" placeholder="Sobrenome" />
                    </div>
                  </div>
                  <div className="form-group">
                    <label>Email</label>
                    <input value={emailContato} onChange={e => setEmailContato(e.target.value)} type="email" placeholder="exemplo@email.com" />
                  </div>
                  <div className="form-row">
                    <div className="form-group">
                      <label>Categoria</label>
                      <select onChange={e => setCategoriaContato(e.target.value)}>
                        <option value="">Selecione</option>
                        <option value="cadastro">Cadastrar Empresa</option>
                    ...
                    <div className="form-group">
                      <label>Assunto</label>
                      <input value={assuntoContato} onChange={e => setAssuntoContato(e.target.value)} type="text" placeholder="Resumo do assunto" />
                    </div>
                  </div>
                  <div className="form-group">
                    <label>Descrição</label>
                    <textarea
                      value={detalhesContato}
                      onChange={e => setDetalhesContato(e.target.value)}
                      rows={4}
